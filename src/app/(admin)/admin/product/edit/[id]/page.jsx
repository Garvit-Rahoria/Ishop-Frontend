...
import { getCategories, getBrands, getColors, getProductById } from "@/api/api-call";
...
export default function AddProduct({ params }) {
    ...
    const submitHandler = (event) => {
        ...
        formData.append("colorId", JSON.stringify(selColor.map(c => c.value)))  
        ...
        client.put(`/product/update/${id}`, formData).then(
            (response) => {
    ...
    const getProduct = async () => {
        try {
            ...
            orginal_price.current.value = data.orginal_price
            final_price.current.value = data.final_price
            discount_percent.current.value = data.discount_percent
            setText(data.long_description)
            setImage(data.thumbnail)
            setShortDesc(data.short_description)

            // Category pre-select
            if (data.categoryId) {
                setSelCategory({ value: data.categoryId._id, label: data.categoryId.name })
            }

            //  Brand pre-select
            if (data.brandId) {
                setSelBrand({ value: data.brandId._id, label: data.brandId.name })
            }

            //  Colors pre-select — {value, label} format mein
            if (data.colorId?.length) {
                setSelColor(data.colorId.map(c => ({ value: c._id, label: c.name })))
            }

            // thumbnail
            setImage(`http://localhost:5000/product/${data.thumbnail}`)
    ...
    return (
        <form onSubmit={submitHandler} className="p-3 bg-gray-100 min-h-screen">
                ...
                <div className="grid md:grid-cols-3 gap-4">
                    <div>
                        ...
                        <Select className="w-full  mt-1 rounded-xl border border-gray-100"
                            ...
                            value={selCategory}                          // ✅ pre-selected
                            onChange={(opt) => setSelCategory(opt)}
                    ...
                    <div>
                        ...
                        <Select className="w-full  mt-1 rounded-xl border border-gray-100"
                            ...
                            value={selBrand}                             // ✅ pre-selected
                            onChange={(opt) => setSelBrand(opt)}
                    ...
                    <div className='flex items-center justify-center'>
                        {image && (
                            <img
                                src={image}
                                alt="category"
                                className="w-30 h-20 object-cover rounded-lg mt-3"
                            />
                        )}
                    </div>

    ...
    const handleOrder = async () => {
        ...
        try {
            ...
            if (!response.data.success) {
                notify(response.data.message || 'Order creation failed', false);
                return;
            }
            ...
            if (paymentMethod === 'online') {
                    ...
                    handler: async (rzpResponse) => {
                        try {
                            ...
                            if (verifyRes.data.success) {
                                dispatch(emptyCart());
                                router.push(`/thank-you?orderId=${verifyRes.data.orderId}`);
                            } else {
                                notify(verifyRes.data.message || 'Payment verification failed', false);
                            }
                        } catch (err) {
                            notify('Payment verification error. Contact support.', false);
                ...
                rzpInstance.on('payment.failed', (failResponse) => {
                    razorpayOpenRef.current = false;
                    setLoading(false);
                    notify(`Payment failed: ${failResponse.error?.description || 'Unknown error'}`, false);
                });
            ...
            <div className="max-w-7xl mx-auto px-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
                    <div className="bg-gray-100 rounded-md p-4 text-sm">
                        Returning customer?{' '}
                        <span className="text-red-500 cursor-pointer">Click here to log in</span>
                    </div>
                    <div className="bg-gray-100 rounded-md p-4 text-sm">
                        Have a coupon?{' '}
                                ...
                                <div className="bg-white/90 backdrop-blur-xl border border-white/40 shadow-2xl rounded-[32px] p-6 md:p-8">
                                    <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-8">
                                        <div>
                                            ...
                                            <p className="text-gray-500 mt-1">Select your preferred delivery location</p>
                                        </div>
                                        <button
                                            ...
                                            >
                                                ...
                                                <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-5">
                                                    <div className="space-y-3">
                                                        <div>
                                                            <h3 className="text-xl font-bold text-gray-800">{addr.fullName}</h3>
                                                            <p className="text-sm text-gray-500">Home Address</p>
                                                        </div>
                                                        <div className="space-y-1">
                                                            <p className="text-gray-700 leading-relaxed">{addr.addressLine}</p>
                            ...
                            <div className="relative overflow-hidden bg-white/90 backdrop-blur-xl border border-white/40 shadow-2xl rounded-[32px] p-6 md:p-7 h-fit">
                                ...
                                <div className="relative flex items-center justify-between mb-6">
                                    <div>
                                        ...
                                        <p className="text-sm text-gray-500 mt-1">Review your final payment details</p>
                                    </div>
                                ...
                                <div className="relative space-y-4">
                                    <div className="flex items-center justify-between bg-gray-50 rounded-2xl px-4 py-3">
                                        <div>
                                            <p className="text-sm font-medium text-gray-500">Original Total</p>
                                            <p className="text-xs text-gray-400 mt-1">Before discounts</p>
                                        </div>
                                        <h3 className="text-lg font-bold text-gray-800">₹{cart.original_total}</h3>
                                    </div>
                                ...
                                <div className="relative bg-gradient-to-r from-gray-900 to-gray-800 rounded-3xl p-5 text-white shadow-xl">
                                    <div className="flex items-center justify-between">
                                        <div>
                                            ...
                                            <h2 className="text-3xl font-bold mt-1">₹{cart.final_total}</h2>
                                        </div>
                                ...
                                <button
                                    disabled={loading || cart.items.length === 0 || addresses.length === 0}
                                    onClick={handleOrder}
                                    className="group relative overflow-hidden w-full mt-7 bg-gradient-to-r from-teal-500 to-emerald-500 hover:shadow-2xl hover:scale-[1.02] transition-all duration-300 text-white py-4 rounded-2xl font-bold text-base disabled:opacity-50 disabled:cursor-not-allowed disabled:scale-100"
                                >
